...
def k ():
    ...
    if clientID!=-1:
     ...
     #   res,gpsY=vrep.simxGetObjectHandle(clientID,'K3_GPSY',vrep.simx_opmode_blocking)
      #  res,gpsZ=vrep.simxGetObjectHandle(clientID,'K3_GPSZ',vrep.simx_opmode_blocking)
        ...
        res,resolution,image=vrep.simxGetVisionSensorImage(clientID,s3,1,vrep.simx_opmode_streaming)
        res,resolution,image=vrep.simxGetVisionSensorImage(clientID,s2,1,vrep.simx_opmode_streaming)
        wposition=vrep.simxGetObjectPosition(clientID,robot,-1,vrep.simx_opmode_streaming)
    ...
    while clientID!=-1:
        ...
        wposition=vrep.simxGetObjectPosition(clientID,robot,-1,vrep.simx_opmode_buffer)
     ...
     #####################################################################
       ...
       ############################
        ...
        while 46 in image:#stop khepera at goal
            if (66 in image3 and 0 in image2):
                m()
            if(66 in image2 and 0 in image3):
                j()
...
def m ():
    ...
    if clientID!=-1:
        ...
        res,s1=vrep.simxGetObjectHandle(clientID,'K3_colorSensorRight#1',vrep.simx_opmode_blocking)
